...
> **补丁必须打在程序真实启动后实际加载的那套文件里。改错目录，即使文件结构看起来正确，也不会生效。**
...
1. 确认这个位置里的 `utils.js` 和 `translate-inject.js` 是否构成完整补丁闭环
2. 确认程序当前版本实际是不是加载这套资源
3. **补丁必须打在程序真实启动后实际加载的那套文件上**
4. 最终以**改完并完全重启后是否真的生效**作为判断标准
